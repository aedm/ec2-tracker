...
use aws_sdk_ec2::model::InstanceType::A1Large;
use aws_sdk_ec2::model::OfferingTypeValues::NoUpfront;
...
async fn show_state(client: &Client) -> Result<(), Error> {
    ...
    for counter in 1.. {
        let resp = client
            ...
            // .set_filters(Some(vec![filter.clone()]))
            // .set_include_marketplace(Some(true))
        ...
        println!(
            "{:3} Offerings: {:?}",
            counter, resp.reserved_instances_offerings
        );
